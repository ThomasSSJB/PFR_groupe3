...
void setup() {
  ...
  // Initialisation des distances pour eviter le blocage au demarrage
  distanceFront = 100;
  distanceBack = 100;
...
void loop() {
  ...
  if (Serial1.available()) {
    ...
    if (commande == '1') {
      mesurerDistanceAvant();          // CORRECTION : mesure avant de tester
      if (distanceFront > 35) {
        avancer();
        avance = true;
        recule = false;
    ...
    if (commande == '2') {
      mesurerDistanceArriere();        // CORRECTION : mesure avant de tester
      if (distanceBack > 35) {
        reculer();
        recule = true;
        avance = false;
